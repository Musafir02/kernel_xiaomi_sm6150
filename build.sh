...
ZIPNAME="vortex-${DEVICE}-$(date '+%Y%m%d-%H%M').zip"

export ARCH=arm64
export KBUILD_BUILD_USER=Musafir02
export KBUILD_BUILD_HOST=build-host
export PATH="$home/inf/prebuilts/clang/host/linux-x86/clang-r547379/bin/:$PATH"
...
if [ -d "$AK3_DIR" ]; then
	...
	if ! git clone -q https://github.com/Musafir02/AnyKernel3 -b master AnyKernel3; then
